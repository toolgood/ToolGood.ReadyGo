...
{
    /// <summary>
    /// 复杂对象（嵌套对象）映射测试（基于 Firebird）
    /// </summary>
    [Collection("FirebirdDb")]
    public class ComplexMappingFirebirdDbTests
    {
        private static FirebirdDbTestDb CreateWithUsers()
        {
            var db = FirebirdDbTestDb.Create();
            var helper = db.Helper;
            helper.Execute("CREATE TABLE \"Users\" (\"UserId\" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, \"Name\" VARCHAR(255), \"Street\" VARCHAR(255), \"City\" VARCHAR(255));");
