...
{
    /// <summary>
    /// SelectOneToMany 一对多查询单元测试（基于 Firebird）
    /// </summary>
    [Collection("FirebirdDb")]
    public class OneToManyFirebirdDbTests
    {
        private static FirebirdDbTestDb CreateWithOrders()
        {
            var db = FirebirdDbTestDb.Create();
            var helper = db.Helper;
            helper.Execute("CREATE TABLE \"Customer\" (\"Id\" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, \"Name\" VARCHAR(255));");
            helper.Execute("CREATE TABLE \"OrderItem\" (\"OrderItemId\" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, \"CustomerId\" INTEGER, \"Product\" VARCHAR(255));");
