...
namespace ToolGood.ReadyGo.LinQ.Expressions
{
    ...
    public class SqlExpression
    {
        ...
        public SqlExpression(SqlHelper helper)
        {
            _sqlHelper = helper;
            var db = helper.GetDatabase();
            _databaseType = db.DatabaseType;
            _pocoDataFactory = db.PocoDataFactory;
        }
        ...
        private object VisitColumnAccessMethod(MethodCallExpression m)
        {
            ...
            switch (m.Method.Name) {
                case "Trim": statement = CreateFunction(SqlFunction.Trim, quotedColName); break;
                case "TrimStart": statement = CreateFunction(SqlFunction.LTrim, quotedColName); break;
                case "TrimEnd": statement = CreateFunction(SqlFunction.RTrim, quotedColName); break;
                case "ToUpper": statement = CreateFunction(SqlFunction.Upper, quotedColName); break;
                case "ToLower": statement = CreateFunction(SqlFunction.Lower, quotedColName); break;
                case "StartsWith": statement = CreateFunction(SqlFunction.Fuction, "{0} LIKE {1}", quotedColName, SqlUtil.ToEscapeLikeParam(wildcardArg) + "%"); break;
                case "EndsWith": statement = CreateFunction(SqlFunction.Fuction, "{0} LIKE {1}", quotedColName, "%" + SqlUtil.ToEscapeLikeParam(wildcardArg)); break;
                case "Contains": statement = CreateFunction(SqlFunction.Fuction, "{0} LIKE {1}", quotedColName, "%" + SqlUtil.ToEscapeLikeParam(wildcardArg) + "%"); break;
                case "Substring":
                    var startIndex = Int32.Parse(_args[0].ToString()) + 1;
                    if (_args.Count == 1) {
                        statement = CreateFunction(SqlFunction.SubString2, quotedColName, startIndex);
                        break;
                    }
                    var length = Int32.Parse(_args[1].ToString());
                    statement = CreateFunction(SqlFunction.SubString3, quotedColName, startIndex, length);
                    break;
                ...
                case "Concat":
                    ...
                    args.AddRange(_args);
                    statement = CreateFunction(SqlFunction.Concat, args.ToArray()); break;
                case "ToString": statement = quotedColName.ToString(); break;
                case "IndexOf": statement = CreateFunction(SqlFunction.IndexOf, quotedColName, _args[0]); break;
                case "Replace": statement = CreateFunction(SqlFunction.Replace, quotedColName, _args[0], _args[1]); break;
        ...
        private string getColumnName(MemberExpression m)
        {
            ...
            if (p == null) return _databaseType.EscapeSqlIdentifier(colName);

            var tableDef = _pocoDataFactory.ForType(p.Type);

            var col = tableDef.Columns.Where(q => q.Value.MemberInfoData != null && q.Value.MemberInfoData.MemberInfo.Name == colName).Select(q => q.Value).FirstOrDefault();
            if (col != null) {
                colName = col.ColumnName;
            }
            return _databaseType.EscapeSqlIdentifier(colName);
        }
                    ...
                    if (p.NodeType == ExpressionType.Convert) {
                        ...
                        if (pp != null) {
                            ...
                            switch (m.Member.Name) {
                                case "Year": sql = CreateFunction(SqlFunction.Year, Visit(m1)); break;
                                case "Month": sql = CreateFunction(SqlFunction.Month, Visit(m1)); break;
                                case "Day": sql = CreateFunction(SqlFunction.Day, Visit(m1)); break;
                                case "Hour": sql = CreateFunction(SqlFunction.Hour, Visit(m1)); break;
                                case "Minute": sql = CreateFunction(SqlFunction.Minute, Visit(m1)); break;
                                case "Second": sql = CreateFunction(SqlFunction.Second, Visit(m1)); break;
                                case "Value": return Visit(m1);
                                case "DayOfWeek": sql = CreateFunction(SqlFunction.WeekDay, Visit(m1)); break;
                                case "DayOfYear": sql = CreateFunction(SqlFunction.DayOfYear, Visit(m1)); break;
        ...
        private string CreateFunction(SqlFunction function, params object[] args)
        {
            switch (function) {
        ...
        private string CreateFunction(string sql, object[] args, int index = 0)
        {
        ...
        private string EscapeParam(object value)
        {
