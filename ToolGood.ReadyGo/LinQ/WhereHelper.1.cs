...
using System.Text;
using ToolGood.ReadyGo.LinQ.Expressions;
using ToolGood.ReadyGo.NPoco;

namespace ToolGood.ReadyGo.LinQ
{
    /// <summary>
    /// 动态Sql拼接
    /// </summary>
    ...
    public partial class WhereHelper<T1> where T1 : class
    {
        internal WhereHelper(SqlHelper helper)
        {
            ...
            this._databaseType = db.DatabaseType;
            this._pocoDataFactory = db.PocoDataFactory;
            this._paramPrefix = this._databaseType.GetParameterPrefix(_sqlhelper._connectionString);
            _sqlExpression = new Expressions.SqlExpression(_sqlhelper);
        }
        ...
        private string CreateSelectHeader(Type outType, List<SelectHeader> defineHeader)
        {
            ...
            if (outType == typeof(object)) {
                ...
                var pd = _pocoDataFactory.ForType(outType);
                StringBuilder sb = new StringBuilder();
        ...
        private List<SelectHeader> GetSelectHeader()
        {
            ...
            var pd = _pocoDataFactory.ForType(typeof(T1));
            foreach (var col in pd.Columns) {
                ...
                header.QuerySql = header.Table + "." + _databaseType.EscapeSqlIdentifier(col.Value.ColumnName);
                list.Add(header);
            }
        ...
        {
            var pd1 = _pocoDataFactory.ForType(typeof(T1));
            StringBuilder sb = new StringBuilder();
            sb.Append("FROM ");
            if (string.IsNullOrEmpty(_table)) {
                sb.Append(_databaseType.EscapeTableName(pd1.TableInfo.TableName));
            } else {
                sb.Append(_table);
            }
        ...
        public int Update(object setData)
        {
            ...
            foreach (var pi in pis) {
                ...
                sb.AppendFormat("{0}=@{1}", _databaseType.EscapeSqlIdentifier(pi.Name), index++);
        ...
        public int Update(IDictionary<string, object> setData)
        {
            ...
            foreach (var item in setData) {
                if (index > 0) { sb.Append(","); }
                sb.AppendFormat("{0}=@{1}", _databaseType.EscapeSqlIdentifier(item.Key), index++);
                args.Add(item.Value);
            }
            var sql = BuildUpdateSql(sb.ToString(), args);
            return _sqlhelper.Update<T1>(sql, _args.ToArray());
