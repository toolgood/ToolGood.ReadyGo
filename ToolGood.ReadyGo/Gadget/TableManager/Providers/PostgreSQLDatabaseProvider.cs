...
namespace ToolGood.ReadyGo.Gadget.TableManager.Providers
{
    ...
    public class PostgreSQLDatabaseProvider : DatabaseProvider
    {
        ...
        private string CreateColumn(TableInfo ti, ColumnInfo ci)
        {
            ...
            if (ci.SerializedAs == ColumnInfo.SerializedKind.String) return CreateField(ti, ci, ci.IsText ? "text" : "varchar", ci.IsText ? "" : (string.IsNullOrEmpty(ci.FieldLength) ? "4000" : ci.FieldLength), false);
            ...
            if (type == typeof(Decimal[])) return CreateField(ti, ci, "numeric[]", ci.FieldLength, false);
            ...
            if (type == typeof(List<Decimal>)) return CreateField(ti, ci, "numeric[]", ci.FieldLength, false);
