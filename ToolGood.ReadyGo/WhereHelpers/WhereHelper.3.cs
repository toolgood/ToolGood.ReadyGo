...
{
    public partial class WhereHelper<T1, T2, T3> : WhereHelperBase, IDeepCloneable<WhereHelper<T1, T2, T3>>
        where T1 : class, new()
        where T2 : class, new()
        where T3 : class, new()
    {
        ...
        #region Select Page SkipTake Single SingleOrDefault First FirstOrDefault
        public List<T> Select<T>(Expression<Func<T1, T2, T3, T>> columns)
        {
            string sql;
            SqlExpression.GetColumns(columns, out sql);
            return _sqlhelper.Select<T>(GetFullSelectSql(sql), _args);
        }
        public T Single<T>(Expression<Func<T1, T2, T3, T>> columns)
        {
            string sql;
            SqlExpression.GetColumns(columns, out sql);
            return _sqlhelper.Single<T>(GetFullSelectSql(sql), _args);
        }
        public T SingleOrDefault<T>(Expression<Func<T1, T2, T3, T>> columns)
        {
            string sql;
            SqlExpression.GetColumns(columns, out sql);
            return _sqlhelper.SingleOrDefault<T>(GetFullSelectSql(sql), _args);
        }
        public T First<T>(Expression<Func<T1, T2, T3, T>> columns)
        {
            string sql;
            SqlExpression.GetColumns(columns, out sql);
            return _sqlhelper.First<T>(GetFullSelectSql(sql), _args);
        }
        public T FirstOrDefault<T>(Expression<Func<T1, T2, T3, T>> columns)
        {
            string sql;
            SqlExpression.GetColumns(columns, out sql);
            return _sqlhelper.FirstOrDefault<T>(GetFullSelectSql(sql), _args);
        }
        public List<T> SkipTake<T>(long skip, long take, Expression<Func<T1, T2, T3, T>> columns)
        {
            string sql;
            SqlExpression.GetColumns(columns, out sql);
            return _sqlhelper.SkipTake<T>(skip, take, GetFullSelectSql(sql), _args);
        }
        public Page<T> Page<T>(long page, long itemsPerPage, Expression<Func<T1, T2, T3, T>> columns)
        {
            string sql;
            SqlExpression.GetColumns(columns, out sql);
        ...
        /// </summary>
        /// <returns></returns>
        public WhereHelper<T1, T2, T3> DeepClone()
        {
            WhereHelper<T1, T2, T3> newWhere = new WhereHelper<T1, T2, T3>(this._sqlhelper);
