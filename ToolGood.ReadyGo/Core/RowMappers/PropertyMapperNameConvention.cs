...
namespace ToolGood.ReadyGo.NPoco.RowMappers
{
    public static class PropertyMapperNameConvention
    {
        ...
        internal static IEnumerable<PosName> ConvertFromOldConvention(this IEnumerable<PosName> posNames, List<PocoMember> pocoMembers)
        {
            ...
            var members = FlattePocoMembers(pocoMembers).ToList();
            var level = 0;
        ...
        private static IEnumerable<PocoMemberLevel> FlattePocoMembers(List<PocoMember> pocoMembers, int levelMonitor = 1)
        {
            foreach (var pocoMember in pocoMembers.OrderBy(x => x.PocoMemberChildren.Count != 0))
            {
                ...
                levelMonitor++;
                foreach (var pocoMemberLevel in FlattePocoMembers(pocoMember.PocoMemberChildren, levelMonitor))
                {
