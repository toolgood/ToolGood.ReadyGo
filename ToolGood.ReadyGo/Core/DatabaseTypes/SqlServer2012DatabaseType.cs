...
{
    /// <summary>
    /// SQL Server 数据库类型实现（2005+，ROW_NUMBER 分页；2012+ 亦适用）。
    /// </summary>
    public class SqlServer2012DatabaseType : DatabaseType
    {
        /// <summary>
        /// 使用方括号包裹 SQL 标识符，内部右方括号转义为“]]”。
        /// </summary>
        ...
        /// <summary>
        /// 构建 SQL Server 分页查询（基于 ROW_NUMBER）。
        /// </summary>
        ...
        {
            // SCOPE_IDENTITY 比 @@IDENTITY 更安全，只返回当前会话/作用域的自增值
            cmd.CommandText += ";\nSELECT SCOPE_IDENTITY();";
        }

        /// <summary>
        /// 执行插入操作，并在指定主键时通过 SCOPE_IDENTITY 返回自增值。
        /// </summary>
        ...
        /// <summary>
        /// 异步执行插入操作，并在指定主键时通过 SCOPE_IDENTITY 返回自增值。
        /// </summary>
        /// <typeparam name="T">POCO 类型。</typeparam>
        /// <param name="db">数据库实例。</param>
        /// <param name="cmd">数据库命令。</param>
