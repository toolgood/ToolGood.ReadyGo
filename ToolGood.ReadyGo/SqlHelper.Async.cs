...
namespace ToolGood.ReadyGo
{
    public partial class SqlHelper
    {
        ...
        /// <summary>
        /// 批量更新，每个对象按主键更新全部列，异步操作。
        /// </summary>
        /// <typeparam name="T">实体类型</typeparam>
        /// <param name="list">要更新的实体集合</param>
        /// <returns>受影响的总行数</returns>
        public async Task<int> UpdateList_Async<T>(List<T> list) where T : class
        {
            if (list == null) throw new ArgumentNullException("list is null.");
            if (list.Count == 0) return 0;

            return await GetDatabase().UpdateBatchAsync(list.Select(x => UpdateBatch.For(x)));
        }

        /// <summary>
        /// 批量更新，仅更新快照中发生变更的列，异步操作。
        /// </summary>
        /// <typeparam name="T">实体类型</typeparam>
        /// <param name="list">要更新的实体集合</param>
        /// <param name="snapshots">与 list 一一对应的快照集合（来自 StartSnapshot）</param>
        /// <returns>受影响的总行数</returns>
        public async Task<int> UpdateList_Async<T>(List<T> list, List<Snapshot<T>> snapshots) where T : class
        {
            if (list == null) throw new ArgumentNullException("list is null.");
            ...
            return await GetDatabase().UpdateBatchAsync(batches);
        }
        ...
        /// <summary>
        /// 批量保存：新对象（主键为默认值）执行批量插入，已存在对象执行批量更新，异步操作。
        /// </summary>
        /// <typeparam name="T">实体类型</typeparam>
        /// <param name="list">要保存的实体集合</param>
        public async Task SaveList_Async<T>(List<T> list) where T : class
        {
            if (list == null) throw new ArgumentNullException("list is null.");
            if (list.Count == 0) return;
