...
{
    /// <summary>
    /// SelectOneToMany 一对多查询单元测试（基于 PostgreSQL）
    /// </summary>
    [Collection("PostgreSQLDb")]
    public class OneToManyPostgreSQLDbTests
    {
        private static PostgreSQLTestDb CreateWithOrders()
        {
            var db = PostgreSQLTestDb.Create();
            var helper = db.Helper;
            helper.Execute("CREATE TABLE \"Customer\" (\"Id\" integer NOT NULL PRIMARY KEY, \"Name\" varchar(255));");
            helper.Execute("CREATE TABLE \"OrderItem\" (\"OrderItemId\" integer NOT NULL PRIMARY KEY, \"CustomerId\" integer, \"Product\" varchar(255));");
