...
{
    /// <summary>
    /// TableManager（SqlHelper._TableHelper）单元测试（基于 PostgreSQL）
    /// </summary>
    [Collection("PostgreSQLDb")]
    public class TableManagerPostgreSQLDbTests
    {
        private static PostgreSQLTestDb CreateTable()
        {
        ...
        [Fact]
        public void GetTryCreateTable_PostgreSQL_包含关键片段()
        {
            ...
            Assert.Contains("\"Id\" integer DEFAULT NEXTVAL('seq_Tb_Order') NOT NULL PRIMARY KEY", sql);
            Assert.Contains("\"OrderNo\" varchar(50)", sql);
            Assert.Contains("\"Remark\" text", sql);
            Assert.Contains("CREATE INDEX IF NOT EXISTS i_Tb_Order_UserId ON \"Tb_Order\"(\"UserId\")", sql);
            Assert.Contains("CREATE UNIQUE INDEX IF NOT EXISTS u_Tb_Order_OrderNo ON \"Tb_Order\"( \"OrderNo\")", sql);
        }
        ...
        public void DropTable_与_GetDropTable()
        {
            ...
            Assert.Contains("DROP SEQUENCE IF EXISTS seq_Tb_Order;", helper._TableHelper.GetDropTable(typeof(Tb_Order)));
