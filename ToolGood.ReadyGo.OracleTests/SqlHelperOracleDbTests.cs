...
{
    /// <summary>
    /// SqlHelper 同步方法单元测试（基于 Oracle）
    /// 注意：Oracle 标识符未加引号时统一视为大写，测试 SQL 使用大写列名与双引号大写表名。
    /// </summary>
    [Collection("OracleDb")]
    public class SqlHelperOracleDbTests
    {
        #region Insert
        ...
        {
            using var db = OracleTestDb.Create();
        ...
        public void Transaction_Commit()
        {
            ...
            using (var trans = helper.UseTransaction()) {
                helper.Execute("INSERT INTO \"USERINFO\" (NAME, AGE, REMARK, CREATETIME, MONEY, ISDELETE) VALUES ('事务1', 1, NULL, DATE '2026-01-01', 1, 1)");
                trans.Complete();
            }
        ...
        public void Transaction_Rollback()
        {
            ...
            using (var trans = helper.UseTransaction()) {
                helper.Execute("INSERT INTO \"USERINFO\" (NAME, AGE, REMARK, CREATETIME, MONEY, ISDELETE) VALUES ('事务2', 1, NULL, DATE '2026-01-01', 1, 1)");
                // 不调用 Complete，释放时应回滚
            }
    ...
    /// <summary>
    /// SqlHelper 异步方法单元测试（基于 Oracle）
    /// </summary>
    [Collection("OracleDb")]
    public class SqlHelperOracleDbAsyncTests
    {
        [Fact]
