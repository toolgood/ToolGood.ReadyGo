...
{
    /// <summary>
    /// SqlHelper 同步方法单元测试（基于 Access）
    /// 注意：Access 仅支持 SELECT TOP 分页（skip=0），翻页（skip&gt;0）由 MsAccessDbDatabaseType 抛异常，因此不含翻页测试。
    /// </summary>
    [Collection("MsAccessDb")]
    public class SqlHelperMsAccessDbTests
    {
        #region Insert
        ...
        {
            using var db = MsAccessDbTestDb.Create();
        ...
        #region Select / FirstOrDefault

        [Fact]
        public void Select_And_FirstOrDefault()
        {
            using var db = MsAccessDbTestDb.Create();
            var helper = db.Helper;
            for (int i = 0; i < 13; i++) {
    ...
    /// <summary>
    /// SqlHelper 异步方法单元测试（基于 Access）
    /// </summary>
    [Collection("MsAccessDb")]
    public class SqlHelperMsAccessDbAsyncTests
    {
        [Fact]
        ...
        [Fact]
        public async Task Select_FirstOrDefault_Async()
        {
            using var db = MsAccessDbTestDb.Create();
            var helper = db.Helper;
            for (int i = 0; i < 13; i++) {
