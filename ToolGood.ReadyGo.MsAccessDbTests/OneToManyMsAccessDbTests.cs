...
{
    /// <summary>
    /// SelectOneToMany 一对多查询单元测试（基于 Access）
    /// </summary>
    [Collection("MsAccessDb")]
    public class OneToManyMsAccessDbTests
    {
        private static MsAccessDbTestDb CreateWithOrders()
        {
            var db = MsAccessDbTestDb.Create();
            var helper = db.Helper;
            helper.Execute("CREATE TABLE [Customer] ([Id] AUTOINCREMENT PRIMARY KEY, [Name] TEXT(255));");
            helper.Execute("CREATE TABLE [OrderItem] ([OrderItemId] AUTOINCREMENT PRIMARY KEY, [CustomerId] INTEGER, [Product] TEXT(255));");
