...
{
    /// <summary>
    /// SqlHelper.Where 链式查询、UpdateMany、DeleteMany 测试（基于 Access）
    /// 注意：Access 仅支持 SELECT TOP 分页（skip=0），ToPage(2,2) 等翻页会抛异常，因此不含翻页测试。
    /// </summary>
    [Collection("MsAccessDb")]
    public class WhereHelperMsAccessDbTests
    {
